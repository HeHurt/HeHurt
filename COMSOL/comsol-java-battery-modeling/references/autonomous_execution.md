...
| 工具 | 典型路径 (Windows) | 作用 |
|---|---|---|
| `java.exe` | `D:\Program Files\COMSOL\COMSOL64\Multiphysics\java\win64\jre\bin\java.exe` | 运行 .class (不是编译) |
| `javac.exe` | `D:\Program Files\COMSOL\COMSOL64\Multiphysics\java\win64\jre\bin\javac.exe` | 编译 .java (注意:JRE 可能没有 javac,需要 JDK) |
| `comsolcompile.exe` | `D:\Program Files\COMSOL\COMSOL64\Multiphysics\bin\win64\comsolcompile.exe` | ★ **推荐**: 一键编译 .java → .class,自动处理 classpath |
| `comsolbatch.exe` | `D:\Program Files\COMSOL\COMSOL64\Multiphysics\bin\win64\comsolbatch.exe` | 无 GUI 跑 .class/.mph |
| `comsol.exe` (Desktop) | `D:\Program Files\COMSOL\COMSOL64\Multiphysics\bin\win64\comsol.exe` | GUI |
...
"<JDK-path>/bin/javac.exe" \
    -cp "D:\Program Files\COMSOL\COMSOL64\Multiphysics\plugins\*" \
    -d <output-dir> \
    GeneratedModel.java
```
...
# == STEP A: 对齐(假设已完成) ==
paths = {
    "comsolcompile": r"D:\Program Files\COMSOL\COMSOL64\Multiphysics\bin\win64\comsolcompile.exe",
    "comsolbatch":   r"D:\Program Files\COMSOL\COMSOL64\Multiphysics\bin\win64\comsolbatch.exe",
