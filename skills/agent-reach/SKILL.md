...
name: agent-reach
description: >
  Use ONLY when the user explicitly invokes `$agent-reach` or explicitly says
  to use agent-reach. Do not trigger automatically for internet research,
  URLs, links, or platform mentions. When explicitly invoked, route read-only
  internet retrieval across supported web and social platforms.
metadata:
...
15 platforms, multiple backends each.

## Activation rule

**Explicit invocation only.** Use this skill only when the user writes
`$agent-reach` or explicitly asks to use agent-reach. Ordinary requests to
search, research, open a URL, or inspect a platform must use other available
tools and must not activate this skill.
