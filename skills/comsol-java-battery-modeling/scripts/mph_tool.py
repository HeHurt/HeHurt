...
def audit_model(model: Any, source: str, options: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    param_patterns = options.get("parameter_patterns")
    variable_patterns = options.get("variable_patterns")
    components = {}
    for component_tag in _tags(model.component()):
        ...
        for physics_tag in _tags(component.physics()):
            ...
            physics[physics_tag] = {
                "label": str(node.label()),
                "selection": _selection(node),
                "features": _feature_tree(node.feature(), include_all),
            }
        components[component_tag] = {
            "physics": physics,
            "variables": _variables(component, variable_patterns),
            "couplings": _couplings(component),
            "probes": _probes(component),
        }
    return {
        ...
        "studies": _study_info(model),
        "results": _results_info(model),
    }
...
def action_audit(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, item in enumerate(config["models"]):
        ...
        try:
            report = audit_model(model, item["path"], config.get("options"))
...
def action_smoke(config: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        node.set("unit", [item.get("unit", "1") for item in expressions])
        ...
        for dataset in datasets:
            try:
                ...
                continue
        if not data:
            raise RuntimeError("Smoke solve completed but no evaluable result dataset was found")
        metrics = {item["name"]: [float(v) for v in values] for item, values in zip(expressions, data)}
    finally:
        try:
            model.result().numerical().remove(numerical_tag)
        except Exception:
