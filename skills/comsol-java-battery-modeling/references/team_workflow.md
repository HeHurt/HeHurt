...
```
只 commit .java,不 commit .mph
```

理由:
- `.mph`是加密二进制,既无法diff,也违反公司加密策略
...
```
hicell-comsol-models/
├── .github/
│   ├── copilot-instructions.md      ← 仓库级AI指令 (本skill的SKILL.md)
│   ├── instructions/
│   │   └── comsol.instructions.md   ← 按.java文件类型生效
│   └── prompts/
│       ├── cp-sweep.prompt.md       ← 可复用的Copilot prompt
│       └── add-thermal.prompt.md
├── models/
│   ├── java/                         ← 唯一的git跟踪源
...
如果你公司想推一套标准命名,可以参考下表。**老模型如果不符合,新增时按规则;不要批量重命名,保留历史一致性**。
...
5. **持续**: 用Copilot/Claude生成代码时,严格按SKILL.md的5步工作流走,**禁止跳过Step 2-3直接生成**
