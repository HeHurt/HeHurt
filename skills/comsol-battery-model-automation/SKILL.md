...
description: 以 Codex 为核心审计、诊断、另存修改、运行和验证 COMSOL Multiphysics 6.4 电池模型。用于已有 .mph 的电压、容量、产热、求解器和参数扫描问题，DLP 包装模型，COMSOL 导出 .java 的定向修改，以及 liion/P2D、1D-3D 电化学-热耦合、CC/CP、comsolcompile/comsolbatch 和结果验收。仅需 MATLAB LiveLink 时改用 comsol-livelink-matlab skill。
---
...
| MATLAB 调度或优化器集成 | 转交 `comsol-livelink-matlab` | 不在本 skill 重复 |
...
- DLP 包装 `.mph` 由 COMSOL 后端 `ModelUtil.load()` 加载；离线 ZIP/MCP standalone 失败不代表模型损坏。
- DLP `.java` 遵循项目 `AGENTS.md` 的白名单读取和写回规则；复杂脚本使用 `exec --file`，避免 PowerShell 内联转义。
