...
```bash
# 港股示例
python ${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py --symbol 0700.HK --market HK

# A 股示例
python ${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py --symbol 600519 --market A

# 美股示例
python ${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py --symbol AAPL --market US
```
...
```bash
# 海尔智家 AH 双重上市，分别取数
python ${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py --symbol 06690.HK --market HK
python ${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py --symbol 600690 --market A
```
...
**取数脚本路径**：`${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py`
...
用户输入（股票名称/代码）
  ...
  │   ├── 0.0.2 调用 ${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py 一键取数
...
- **v1.6（2026-05-05）** — 🟢🟢🟢🟢 **API 优先取数双引擎**：
  ...
  - 新增 **`${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py` 一键取数脚本**：输出标准 JSON 到 `./account/_temp_value_analysis_<symbol>.json`，含 9 大字段组（price/valuation/profitability/financials/dividend/growth/shares/company/analyst）
