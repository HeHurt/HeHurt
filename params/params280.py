...
LFP_ocp_charge = build_ocp_function("LFP.csv", name="Pos_OCP", offset=0.01)
LFP_ocp_discharge = build_ocp_function("LFP.csv", name="Pos_OCP", offset=-0.01)
...
def get_hithium_params(t_factor=1,temperature=298.15):
    ...
    cracking_rate_Ai2020 = build_cracking_rate(t_factor, base_rate=0.5 * 2.1939307272313407e-22)
    ...
    hithium_params = build_lfp_cell_params(temperature, {
        # 电池几何参数
        "Negative electrode thickness [m]": 5.4e-05,
        "Separator thickness [m]": 1.6e-05,
        "Positive electrode thickness [m]": 7.0e-05,
        "Negative particle radius [m]": 7.26e-6,
        "Positive particle radius [m]": 3.83e-7,
        "Electrode height [m]": 17.048 * 2,
        "Electrode width [m]": 0.1875*2,
        
        # 电池容量参数
        "Nominal cell capacity [A.h]": 280,
        "Maximum concentration in negative electrode [mol.m-3]": 28263,
        "Maximum concentration in positive electrode [mol.m-3]": 19261,
        
        # 电极结构参数
        "Positive electrode porosity": 0.311,
        "Positive electrode active material volume fraction": 0.6766,
        "Negative electrode porosity": 0.318,
        "Negative electrode active material volume fraction": 0.6598,
        'Separator porosity': 0.42,
        
        # 初始条件
        "Initial concentration in negative electrode [mol.m-3]": 28263 * 0.05,
        "Initial concentration in positive electrode [mol.m-3]": 19261 * 0.95,
        ...
        "Negative electrode LAM constant proportional term [s-1]": 2e-8 * t_factor,
