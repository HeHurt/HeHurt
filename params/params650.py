...
def get_hithium_params(t_factor=1,temperature=298.15):
    ...
    hithium_params = build_lfp_cell_params(temperature, {
        # 电池几何参数（650 电芯，310mm 宽度设计，参照 650不同SOH发热功率仿真需求收集信息.xlsx）
        "Negative electrode thickness [m]": 6.88E-5,     # 阳极单面极片厚度（反弹后）68.8 μm
        "Separator thickness [m]": 9.5e-06,              # 隔膜厚度 9.5 μm
        "Positive electrode thickness [m]": 8.98E-5,     # 阴极单面极片厚度（反弹后）89.8 μm
        "Negative particle radius [m]": 5.5E-6,          # 保持 587 原值（表未提供）
        "Positive particle radius [m]": 4.75E-7,         # 保持 587 原值（表未提供）
        "Electrode height [m]": 0.2015,                  # JR极片宽度 201.5 mm
        "Electrode width [m]":  13.402318*2*4,           # JR极片长度 13402.318 mm × 双面 × JR个数4
        
        # 电池容量参数
        "Nominal cell capacity [A.h]": 650,              # 设计容量 650 Ah
        "Maximum concentration in negative electrode [mol.m-3]": 29094,
        "Maximum concentration in positive electrode [mol.m-3]": 18897,
        
        # 电极结构参数（保持 587 原值，表未提供孔隙率/体积分数）
