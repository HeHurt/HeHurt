"""开启接触电阻后，计算 params587 与 paramsMIC（局部采用587动力学）的10 s恒功率充电峰值。"""
...
run_id = datetime.now().strftime("%Y%m%d_%H%M%S") + "_MIC_override_587_contact_on_20C_98SOC_10s_charge_peak_power"
...
config = {
    ...
    "MIC_local_override_source": "params587 at the same temperature",
    "MIC_local_override_keys": LOCAL_OVERRIDE_KEYS,
...
def build_case_parameters(case_name):
    params587 = get_params587(t_factor=1, temperature=TEMPERATURE_K)
    if case_name == "587Ah":
        return params587
    if case_name == "MIC_local_587_kinetics":
        cell_params = get_paramsMIC(t_factor=1, temperature=TEMPERATURE_K)
        for key in LOCAL_OVERRIDE_KEYS:
            cell_params[key] = params587[key]
        return cell_params
...
for case_name, parameter_module in [
    ("587Ah", "params587"),
    ("MIC_local_587_kinetics", "paramsMIC + local params587 override"),
]:
...
labels = ["587Ah", "MIC 1175Ah (587 kinetics override)"]
...
fig.suptitle("20°C, 98% SOC, 10 s constant-power charge (MIC uses 587 kinetics)")
fig.tight_layout()
