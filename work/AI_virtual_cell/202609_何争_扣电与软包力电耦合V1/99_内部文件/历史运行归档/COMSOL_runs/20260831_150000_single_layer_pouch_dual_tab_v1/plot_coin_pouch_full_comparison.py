...
POUCH_POST = POUCH_RUN / "postprocess_v7_capacity_consistent"
POUCH_DATA = POUCH_POST / "exported_data"
COIN_RUN = ROOT / "20260820_140000_coin_v9_nonlinear_breathing_cycle"
COIN_DATA = COIN_RUN / "exported_data"
PLOTS = POUCH_POST / "plots"
...
pouch = json.loads((POUCH_POST / "pouch_capacity_consistent_full_cycle_comparison.json").read_text(encoding="utf-8"))
...
for series, label, color, style in (
    (cb, "扣电：100 N，无呼吸", "#6b7280", "--"),
    (cc, "扣电：100 N，非线性呼吸", "#1d4ed8", "-"),
    (pb, "软包：0 N 基线", "#f59e0b", "--"),
    (pc, "软包：100 N，非线性呼吸", "#dc2626", "-"),
):
    axes[0].plot(arr(series, "time_s") / 3600, arr(series, "voltage_V"), style, color=color, lw=1.8, label=label)
axes[0].axhline(3.65, color="#be123c", lw=1, ls=":")
axes[0].axhline(2.50, color="#be123c", lw=1, ls=":")
axes[0].set(xlabel="时间 (h)", ylabel="加载端电压 (V)", title="完整 0.5C 充电–静置–放电")
...
for series, label, color, style in (
    ...
    for phase, suffix in (("charge", "充电"), ("discharge", "放电")):
        ...
        axes[1].plot(100 * x, y, style if phase == "charge" else "--", color=color, lw=1.8, label=f"{label}-{suffix}")
axes[1].axhline(3.65, color="#be123c", lw=1, ls=":")
axes[1].axhline(2.50, color="#be123c", lw=1, ls=":")
axes[1].set(xlabel="实际充电 SOC (%)", ylabel="加载端电压 (V)", title="同一截止电压下的可用 SOC 窗口")
axes[1].grid(alpha=0.25)
axes[1].legend(frameon=False)
fig.suptitle("扣电与单层双侧出极耳软包：完整循环电压对比", fontsize=14, weight="bold")
plot_files.append(save(fig, "01_voltage_cycle_coin_vs_pouch.png"))

# 2. Performance metric comparison.
coin_m, pouch_m = coin_cpl["metrics"], pouch_cpl["metrics"]
coin_usable = coin_m["charge_capacity_Ah"] / 0.016398503865463064 * 100
pouch_i1c = max(abs(value) for value in pc["current_A"]) / 0.5
pouch_usable = pouch_m["charge_capacity_Ah"] / pouch_i1c * 100
metrics = [
    ("截止可用容量\n(% 1C容量)", coin_usable, pouch_usable),
...
width = 0.34
bars1 = ax.bar(x - width / 2, [item[1] for item in metrics], width, color="#1d4ed8", label="扣电耦合 100 N")
bars2 = ax.bar(x + width / 2, [item[2] for item in metrics], width, color="#dc2626", label="软包耦合 100 N")
ax.set_xticks(x, [item[0] for item in metrics])
ax.set_ylim(0, 110)
ax.set_ylabel("数值 (%)")
ax.set_title("性能指标：按实际极片厚度重标定 0.5C 后的同口径对比")
ax.grid(axis="y", alpha=0.25)
ax.legend(frameon=False)
ax.bar_label(bars1, fmt="%.1f", padding=3)
ax.bar_label(bars2, fmt="%.1f", padding=3)
plot_files.append(save(fig, "02_performance_metrics_coin_vs_pouch.png"))

# 3. Coupled mechanics comparison.
...
axes[0, 1].set(xlabel="实际 SOC (%)", ylabel="平均孔隙率", title="压力–孔隙率反馈")
...
axes[1, 0].set(xlabel="实际 SOC (%)", ylabel="呼吸应变 (%)", title="非线性嵌锂呼吸")
axes[1, 0].grid(alpha=0.25); axes[1, 0].legend(frameon=False, ncol=2)

coin_disp = arr(cc, "top_disp_um")
pouch_disp = arr(pc, "top_displacement_um")
axes[1, 1].plot(arr(cc, "time_s") / 3600, coin_disp - coin_disp[0], color="#1d4ed8", label="扣电")
axes[1, 1].plot(arr(pc, "time_s") / 3600, pouch_disp - pouch_disp[0], color="#dc2626", label="软包")
axes[1, 1].set(xlabel="时间 (h)", ylabel="相对顶部位移 (μm)", title="厚度方向位移响应")
axes[1, 1].grid(alpha=0.25); axes[1, 1].legend(frameon=False)
fig.suptitle("力学–压力–孔隙率–呼吸双向耦合对比", fontsize=14, weight="bold")
plot_files.append(save(fig, "03_mechanics_coupling_coin_vs_pouch.png"))

# 4. Electrolyte concentration span and lithiation states.
fig, axes = plt.subplots(2, 2, figsize=(13.2, 8.4), constrained_layout=True)
fig.set_constrained_layout_pads(w_pad=0.08, h_pad=0.10, wspace=0.08, hspace=0.14)
...
axes[0, 0].set(xlabel="实际 SOC (%)", ylabel="电解液浓度极差 (mol m$^{-3}$)", title="浓差极化表征")
axes[0, 0].grid(alpha=0.25); axes[0, 0].legend(frameon=False)

axes[0, 1].plot(100 * arr(cc, "actual_SOC"), arr(cc, "theta_neg"), color="#1d4ed8", label="扣电-石墨")
axes[0, 1].plot(100 * arr(cc, "actual_SOC"), arr(cc, "theta_pos"), color="#1d4ed8", ls="--", label="扣电-LFP")
axes[0, 1].plot(100 * arr(pc, "actual_SOC"), arr(pc, "theta_neg"), color="#dc2626", label="软包-石墨")
axes[0, 1].plot(100 * arr(pc, "actual_SOC"), arr(pc, "theta_pos"), color="#dc2626", ls="--", label="软包-LFP")
axes[0, 1].set(xlabel="实际 SOC (%)", ylabel="平均嵌锂比例", title="正负极嵌锂状态")
axes[0, 1].grid(alpha=0.25); axes[0, 1].legend(frameon=False, ncol=2)

for base, coupled, label, color in ((cb, cc, "扣电", "#1d4ed8"), (pb, pc, "软包", "#dc2626")):
    n = min(len(base["time_s"]), len(coupled["time_s"]))
    # Interpolate baseline voltage onto coupled output times for a clean mechanism delta.
...
axes[1, 0].set(xlabel="时间 (h)", ylabel="耦合引起的电压差 (mV)", title="加入压力反馈与呼吸后的电压变化")
...
axes[1, 1].set_ylabel("集流体电流密度 P99 (A m$^{-2}$)")
axes[1, 1].set_title("50% SOC 的集流体电流集中程度")
...
plot_files.append(save(fig, "04_electrochemical_nonuniformity_coin_vs_pouch.png"))

# 5. Pouch current-density maps at representative stages.
stage_specs = [
    ("charge", "SOC10", "充电 10% SOC"),
    ("charge", "SOC50", "充电 50% SOC"),
    ("charge", "SOC85", f"充电截止附近 {pouch_m['maximum_SOC_pct']:.1f}% SOC"),
    ("discharge", "SOC50", "放电 50% SOC"),
    ("discharge", "SOC10", "放电 10% SOC"),
]
loaded = []
all_values = []
...
for ax, (x_mm, y_mm, values, title) in zip(axes.flat, loaded):
    ...
    ax.set_title(title)
    ax.set_xlabel("长度方向 x (mm)")
    ax.set_ylabel("厚度方向 y (mm)")
    ax.grid(alpha=0.15)
axes.flat[-1].axis("off")
fig.colorbar(scatter, ax=axes.ravel().tolist(), label="|集流体电流密度| (A m$^{-2}$)", shrink=0.9)
fig.suptitle("软包双侧出极耳：集流体电流密度空间分布", fontsize=14, weight="bold")
plot_files.append(save(fig, "05_pouch_collector_current_density_maps.png"))

# 6. Spatial pressure/porosity along the pouch length at charge 50% SOC.
...
for name, label, color in (("neg", "负极", "#2563eb"), ("sep", "隔膜", "#059669"), ("pos", "正极", "#dc2626")):
    ...
    for ax, rows, field in ((axes[0], pressure_rows, f"pressure_{name}_kPa"), (axes[1], porosity_rows, f"porosity_{name}")):
        ...
        if field.startswith("pressure"):
            # Eval field data are returned in the model base unit Pa.
            y = y / 1000
...
axes[0].set(xlabel="长度方向 x (mm)", ylabel="局部压缩压力 (kPa)", title="50% SOC 局部压力")
axes[1].set(xlabel="长度方向 x (mm)", ylabel="局部孔隙率", title="50% SOC 压力反馈后的孔隙率")
for ax in axes:
    ax.grid(alpha=0.25); ax.legend(frameon=False)
fig.suptitle("软包局部压力–孔隙率：空间非均匀性", fontsize=14, weight="bold")
plot_files.append(save(fig, "06_pouch_pressure_porosity_spatial_SOC50.png"))

# Quantitative summary.
summary = {
    "coin_coupled_100N": {
        "usable_charge_capacity_pct_of_1C": coin_usable,
        **coin_m,
        "collector_current_density_p99_SOC50_A_m2": coin_j99,
    },
    "pouch_coupled_100N": {
        "usable_charge_capacity_pct_of_1C": pouch_usable,
        **pouch_m,
        "collector_current_density_p99_SOC50_A_m2": pouch_j99,
    },
    "pouch_minus_coin": {
        "usable_capacity_percentage_point": pouch_usable - coin_usable,
        "coulombic_efficiency_percentage_point": pouch_m["coulombic_efficiency_pct"] - coin_m["coulombic_efficiency_pct"],
        "energy_efficiency_percentage_point": pouch_m["energy_efficiency_pct"] - coin_m["energy_efficiency_pct"],
        "pressure_peak_pct": 100 * ((pouch_m["pressure_peak_kPa"] / 1000) / coin_m["pressure_peak_MPa"] - 1),
        "displacement_amplitude_pct": 100 * (
            ((pouch_m["top_displacement_range_um"][1] - pouch_m["top_displacement_range_um"][0]) /
             (coin_m["top_displacement_range_um"][1] - coin_m["top_displacement_range_um"][0])) - 1
        ),
    },
    "plots": plot_files,
}
(POUCH_POST / "coin_pouch_quantitative_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

with (POUCH_POST / "coin_pouch_metrics.csv").open("w", newline="", encoding="utf-8-sig") as handle:
    writer = csv.writer(handle)
    writer.writerow(["metric", "coin_coupled_100N", "pouch_coupled_100N", "unit"])
    writer.writerow(["usable_charge_capacity", coin_usable, pouch_usable, "% of 1C capacity"])
...
print(json.dumps({"ok": True, "plots": plot_files, "summary": str(POUCH_POST / "coin_pouch_quantitative_summary.json")}, ensure_ascii=False))
