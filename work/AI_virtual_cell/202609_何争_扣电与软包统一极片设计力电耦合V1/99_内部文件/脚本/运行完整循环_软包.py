...
OUTPUT = TASK / "02_模型" / "软包_统一极片设计_0p5C_同名义压力_完整耦合_已计算.mph"
RESULT = TASK / "99_内部文件" / "JSON" / "软包_完整循环计算结果.json"
PROGRESS = TASK / "99_内部文件" / "运行记录" / "软包_完整循环.log"
TAG = "pouch_unified_full_cycle"
...
try:
    ...
    model.param().set("SOC_init", "0.01")
    model.param().set("F_ext", "p_compare*Ac")
    model.param().set("C_rate", "0.5")
