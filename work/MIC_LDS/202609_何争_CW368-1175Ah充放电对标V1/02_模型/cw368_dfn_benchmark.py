"""CW368-1175Ah constant-power DFN voltage benchmark workflow."""
...
TASK_ROOT = (
    ...
    / "work"
    / "MIC_LDS"
    / "202609_何争_CW368-1175Ah充放电对标V1"
)
PARAMS_ROOT = PROJECT_ROOT / "params"
MODEL_DIR = TASK_ROOT / "02_模型"
RUN_ID = "20260904_100448_CW368_1175Ah_25C"
DEFAULT_OUTPUT_DIR = (
    PROJECT_ROOT
    / "BatteryProject"
    / "output"
    / "runs"
    / "cw368_1175_benchmark"
    / RUN_ID
)
FINAL_FIGURE_DIR = DEFAULT_OUTPUT_DIR / "plots"
EXPERIMENT_FILES = (
    TASK_ROOT / "03_输入数据" / "CW368_00029_CH2_充放电曲线.csv",
    TASK_ROOT / "03_输入数据" / "CW368_00030_CH1_充放电曲线.csv",
)
...
TEMPERATURE_C = 25
RATED_CAPACITY_AH = 1175.0
NOMINAL_VOLTAGE_V = 3.2
SOURCE_RATE_MAP = {0.125: 0.167, 0.25: 0.333}
RATES = (0.167, 0.333)
...
def load_experiment_curves(
    ...
    for file_path in files:
        ...
        frame["p_rate"] = frame["source_p_rate"].map(SOURCE_RATE_MAP)
        for (p_rate, direction, cycle), raw in frame.groupby(
            ...
            if curve["capacity_ah"].iloc[-1] < 0.5 * RATED_CAPACITY_AH:
                continue
            keep = [
                "time_s",
                "capacity_ah",
            ...
            curve["sample_role"] = (
                "calibration" if "00029" in sample else "validation"
            )
            keep.append("sample_role")
...
def measured_power_targets(
    ...
    """Return the median measured power target for each measured P-rate."""
    targets = {}
...
def run_case(
    ...
    effective_area_factor: float = BENCHMARK_AREA_FACTOR,
    series: str = "dfn_sim",
    calculation_basis: str = "physical_195p5mm_geometry",
) -> pd.DataFrame:
...
def plot_voltage_benchmark(
    grouped: dict[tuple[float, str], list[pd.DataFrame]],
    simulations: dict[tuple[float, str], pd.DataFrame],
    figure_dir: Path = FINAL_FIGURE_DIR,
) -> Path:
    """Plot calibration and independent-validation curves in four panels."""
    ...
    for col, p_rate in enumerate(RATES):
        for row, direction in enumerate(("充电", "放电")):
            ax = axes[row, col]
            for curve in grouped[(p_rate, direction)]:
                role = str(curve["sample_role"].iloc[0])
                sample = str(curve["sample"].iloc[0])
                cycle = int(curve["cycle"].iloc[0])
                color = "#7f7f7f" if role == "calibration" else "#2a9d8f"
                ax.plot(
                    curve["capacity_ah"],
                    curve["voltage_v"],
                    ls="--",
                    ...
                    color=color,
                    alpha=0.85,
                    label=f"{sample} cycle{cycle} ({role})",
                )
            sim = simulations[(p_rate, direction)]
            ax.plot(
                sim["capacity_ah"],
                sim["voltage_v"],
                color="black",
            ...
            english_direction = "Charge" if direction == "充电" else "Discharge"
            ax.set_title(f"25°C {p_rate:.3f}P {english_direction}")
            ax.set_xlabel("Capacity [Ah]")
            ax.set_ylabel("Voltage [V]")
            ax.grid(alpha=0.25)
            ax.legend(fontsize=7)
    path = figure_dir / "CW368-1175Ah_0.167P_0.333P_voltage_benchmark.png"
...
def run_benchmark(
    ...
    """Run the four measured CW368-1175Ah charge/discharge benchmarks."""
    ...
    for index, (p_rate, direction) in enumerate(
        ...
        sim_curve = run_case(
            ...
            series="dfn_sim",
            calculation_basis="physical_195p5mm_geometry",
        )
        simulations[(p_rate, direction)] = sim_curve
        curve_frames.append(sim_curve)
        heat_rows.append(heat_summary(sim_curve))
            ...
            metric_rows.append(
                {
                    ...
                    "sample_role": exp_curve["sample_role"].iloc[0],
                    "status": "ok",
                    **metrics,
                }
            )
    ...
    means = (
        benchmark_metrics.groupby(
            ["temperature_c", "p_rate", "direction", "sample_role"],
            as_index=False,
        )[
            [
                "n_overlap",
    ...
    figure_path = plot_voltage_benchmark(grouped, simulations)
    ...
    config = {
        ...
        "rates": list(RATES),
        "source_rate_map": SOURCE_RATE_MAP,
    ...
    logger.info("Voltage benchmark figure: %s", figure_path)
    return {
        "curves": curves,
        "metrics": metrics,
        "heat_summary": heat,
