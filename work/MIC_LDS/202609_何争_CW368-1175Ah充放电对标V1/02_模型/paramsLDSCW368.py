"""PyBaMM baseline parameter set for the CW368 1175 Ah LFP cell.
...
SINGLE_ELECTRODE_HEIGHT_M = 0.1955
DESIGN_ELECTRODE_WIDTH_M = SINGLE_ELECTRODE_HEIGHT_M * 83 * 2 * 2
EFFECTIVE_AREA_FACTOR = 1.0
NEGATIVE_OCP_STO_WARP_DELTA = 0.0
NEGATIVE_OCP_STO_WARP_START = 0.212309
NEGATIVE_OCP_STO_WARP_END = 0.6
NEGATIVE_OCP_STO_WARP_SMOOTHING = 0.06011121
NEGATIVE_OCP_VOLTAGE_OFFSET_V = 0.0
...
def get_hithium_params(
    ...
    """Return CW368-1175Ah overrides with the physical 195.5 mm geometry."""
    ...
    return build_lfp_cell_params(
        temperature,
        {
            # Geometry: retain the 1300 Ah electrode design except that the
            # single-sheet height is reduced from 213.5 mm to 195.5 mm.
            ...
            # PyBaMM's width stores the total active-sheet height after the
            # winding/layer multipliers; the physical sheet length remains
            # represented by ``Electrode height [m] = 0.555`` above.
            "Electrode width [m]": (
                DESIGN_ELECTRODE_WIDTH_M * effective_area_factor
            ),
            # Capacity and concentrations.
            "Nominal cell capacity [A.h]": RATED_CAPACITY_AH,
