"""CW368-1300Ah 25°C constant-power voltage, capacity, and efficiency sweep."""
...
TASK_ROOT = (
    ...
    / "work"
    / "MIC_LDS"
    / "202609_何争_CW368-1300Ah多倍率容量电压与能效V1"
)
MODEL_DIR = TASK_ROOT / "02_模型"
RUN_ID = "20260904_144254_CW368_1300Ah_25C_multirate"
RUN_DIR = (
    PROJECT_ROOT
    / "BatteryProject"
    / "output"
    / "runs"
    / "cw368_1300_multirate"
    / RUN_ID
)
...
def fresh_parameter_values() -> pybamm.ParameterValues:
    """Create a fresh 25°C parameter object for each case."""
...
def run_case(p_rate: float, direction: str) -> tuple[pd.DataFrame, dict]:
    ...
    simulation = pybamm.Simulation(
        build_model(),
        parameter_values=fresh_parameter_values(),
...
def plot_voltage_curves(curves: pd.DataFrame, direction: str, path: Path) -> None:
    ...
    for color, p_rate in zip(colors, P_RATES):
        ...
        ax.plot(
            ...
            lw=2,
            color=color,
            label=f"{p_rate:g}P ({RATED_CAPACITY_AH * NOMINAL_VOLTAGE_V * p_rate:.2f} W)",
        )
    english = "Charge" if direction == "充电" else "Discharge"
    ax.set_title(f"CW368-1300Ah 25°C Constant-Power {english}")
...
def plot_efficiency(efficiency: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("CW368-1300Ah 25°C Round-Trip Energy Efficiency")
...
def run_sweep() -> dict[str, pd.DataFrame]:
    ...
    plot_voltage_curves(curves, "充电", plot_dir / "CW368-1300Ah_25C_charge_capacity_voltage.png")
    plot_voltage_curves(curves, "放电", plot_dir / "CW368-1300Ah_25C_discharge_capacity_voltage.png")
    plot_efficiency(efficiency, plot_dir / "CW368-1300Ah_25C_energy_efficiency.png")
