...
const ROOT = 'D:/Users/hez/Desktop/hithium/scratch/314_soh_heat_report_20260805';
const STARTER = 'C:/Users/hez/AppData/Local/Temp/codex-hithium/314-report-20260805/template-starter.pptx';
const OUT = 'C:/Users/hez/AppData/Local/Temp/codex-hithium/314-report-20260805/314Ah_soh_heat_report_final_plain.pptx';
const PLAIN_ROOT = 'C:/Users/hez/AppData/Local/Temp/codex-hithium/314-report-20260805';
...
rewrite(
  ...
  'sh/cz2pk3eh',
  '欧盟新电池法MIC电芯100%SOC\n日历老化自放电率仿真报告',
  '314Ah电芯不同SOH下\n0.25P/0.5P充放电产热功率仿真报告',
);
rewrite(presentation, 'sh/ih0nah8r', 'RE_HC_314Ah_Cell_Base ', 'RE_HC_314Ah_Cell_Base ');
notes(presentation.slides.getItem(0), [
  '- Template: D:/Users/hez/Desktop/mic/RE_HCMICBase_E212_新电池法MIC日历老化自放电率性能仿真报告对外输出版.pptx',
  '- Task: C:/HithiumSSD/hithium/work/314Ah/202607_何争_314初始产热实测对标V1',
]);
...
notes(presentation.slides.getItem(3), [
  '- D:/Users/hez/Desktop/hithium/BatteryProject/output/314Ah_0.25P全生命周期产热/正式结果/314Ah_0.25P全生命周期产热结果.xlsx',
  '- D:/Users/hez/Desktop/hithium/BatteryProject/output/314Ah_0.5P全生命周期产热/正式结果/314Ah_0.5P全生命周期产热结果.xlsx',
]);

// Slide 5: BOL benchmark.
rewrite(presentation, 'sh/jadwja98', '模型对标', 'BOL产热实测对标');
await replaceImage(presentation, 'im/qt4by54v', 'bol_benchmark.png', '314Ah BOL产热实测与仿真对比');
await replaceImage(presentation, 'im/buxsralg', 'bol_components.png', '314Ah BOL产热分项');
...
setTable(element(presentation, 'tb/h0zm50ni'), [[
  '对标结论',
  '2.50～3.65 V实测下，校准后平均绝对误差为12.33%；0.5P充电误差最低（5.05%），0.25P放电误差最高（20.54%）。\n目标2.70～3.65 V窗口无直接实测点，报告结果属于经BOL对标后的模型预测。',
]]);
notes(presentation.slides.getItem(4), [
  '- C:/HithiumSSD/hithium/BatteryProject/output/runs/bol_heat_benchmark/20260722_162319_314Ah_BOL无老化_2p5V_2p7V/metrics.csv',
  '- C:/HithiumSSD/hithium/BatteryProject/output/runs/bol_heat_benchmark/20260722_162319_314Ah_BOL无老化_2p5V_2p7V/config.json',
]);
...
rewrite(
  ...
  '0.25P与0.5P均计算至约68%～69%SOH；60%SOH采用90%至最低实算SOH区间的二次拟合外推',
);
rewrite(presentation, 'sh/o7i10zet', '仿真过程说明', '不同倍率下产热随SOH变化');
rewrite(
  presentation,
  'sh/9kfq9cji',
  '通过模型输出的电压-时间曲线，可以直观反映电池在长期循环及静置过程中的电压变化趋势',
  'SOH下降使不可逆热增加，放电侧增幅高于充电侧；0.5P产热约为0.25P的2.8～3.1倍',
);
await replaceImage(presentation, 'im/xo36hkbq', 'trend_025.png', '0.25P产热随SOH变化');
await replaceImage(presentation, 'im/cnup8ful', 'trend_05.png', '0.5P产热随SOH变化');
await replaceImage(presentation, 'im/bmlofat0', 'endpoint_60.png', '60%SOH外推产热功率');
await replaceImage(presentation, 'im/alc765cf', 'growth.png', '100%至60%SOH产热增长率');
await replaceImage(presentation, 'im/pk36dkbu', 'rate_ratio.png', '0.5P与0.25P产热倍率倍数');
setTable(element(presentation, 'tb/jmh8rmto'), [[
  '仿真结论',
  '60%SOH外推：0.25P充/放电产热为6.36/6.40 W，0.5P为18.19/19.64 W。\n相对100%SOH，放电产热增幅为31.41%（0.25P）和34.95%（0.5P），高于对应充电增幅。',
]]);
notes(presentation.slides.getItem(5), [
  '- D:/Users/hez/Desktop/hithium/BatteryProject/output/314Ah_0.25P全生命周期产热/正式结果/314Ah_0.25P全生命周期产热结果.xlsx',
  '- D:/Users/hez/Desktop/hithium/BatteryProject/output/314Ah_0.5P全生命周期产热/正式结果/314Ah_0.5P全生命周期产热结果.xlsx',
  '- Extrapolation: quadratic least-squares fit over SOH 90% to minimum simulated SOH',
]);
...
rewrite(
  ...
  '倍率效应在全SOH区间保持稳定：0.5P产热显著高于0.25P，且低SOH放电侧上升更快',
);
rewrite(presentation, 'sh/ud4j2hsv', '内部老化参数分析', '倍率效应与关键SOH结果');
await replaceImage(presentation, 'im/i9cnmto7', 'compare_charge.png', '充电产热倍率与SOH对比');
await replaceImage(presentation, 'im/jal4vy5s', 'compare_discharge.png', '放电产热倍率与SOH对比');
setTable(element(presentation, 'tb/hwnql87q'), [[
  '倍率效应',
  '100%SOH时，0.5P充/放电产热分别为0.25P的2.79/2.99倍；60%SOH外推后增至2.86/3.07倍。\n倍率升高使欧姆热、反应热和接触热同步放大；SOH下降后极化增强，放电侧产热增长更明显。',
]]);
...
rewrite(presentation, 'sh/h8ji1sfe', '2025/6/14', '2026/8/6');
rewrite(presentation, 'sh/w7ah8nyt', '9', '8');
element(presentation, 'sh/4vahcnyp').text.set([
  { runs: [{ run: '1. 工况：', textStyle: { bold: true } }, '314Ah、25℃等温、2.70～3.65 V，0.25P/0.5P恒功率充放电。'] },
  { runs: [{ run: '2. 对标：', textStyle: { bold: true } }, '2.50～3.65 V BOL产热实测对标，校准后平均绝对误差12.33%。'] },
  { runs: [{ run: '3. 结果：', textStyle: { bold: true } }, '60%SOH外推：0.25P为6.36/6.40 W，0.5P为18.19/19.64 W（充/放电）。'] },
  { runs: [{ run: '4. 机理：', textStyle: { bold: true } }, '倍率升高放大欧姆热与反应热；SOH下降增强极化，放电产热增幅达到31%～35%。'] },
  { runs: [{ run: '5. 边界：', textStyle: { bold: true } }, '实算最低SOH为69.00%（0.25P）和67.80%（0.5P）；60%结果为二次拟合外推，需低SOH实测复核。'] },
]);
